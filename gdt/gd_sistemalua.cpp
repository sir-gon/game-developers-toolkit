...
void GD_SistemaLua::Inicializar(void)
{
    ...
    // Ponemos que se muestren los errores automáticamente
    this->bAutoErrores = true;
    
    
    // Cargamos todas las librerías estandar
    ...
    // Por un "extraño" error el primver valor que se coje de una tabla siempre muestra 0.
...
{
    int iError;
    // Cargamos la cadena y la ejecutamos en modo seguro. Si los errores están automáticos se muestran.
...
{
    int iError;
    // Cargamos el archivo y lo ejecutamos en modo seguro. Si los errores están automáticos se muestran.
...
lua_Number GD_SistemaLua::RetornarNumero(char *strCadenaTabla)
{
    ...
    // Obtenemos la posición del valor en la pila
    int nPosValorPila = lua_gettop(this->LuaMV);

    // Comprobamos si es un número. Si no, mostramos un error.
...
{  
    // Contamos el número de partes en que se divide la cadena 
    char *ptrC;
    ...
    // Obtenemos el número de elementos que despues cojeremos como tabla, y le suamos uno ya que habrá que cojerlo de allí
    ...
    // Ahora nos queda cojer la posición de la clave y cambiar su valor
    ptr = strtok( NULL, this->strToken );
...
{
    // Limpiamos la pila por precacución
    lua_pop(this->LuaMV, lua_gettop(this->LuaMV) );
...
{
    int iError;
    // Llamamos a la función en modo seguro en modo seguro. Si los errores están automáticos se muestran.
...
{
    // Cambiamos si los errores se mostrarán automáticamente
